...
#create figure
pdf(paste(outDir, "simCompMethodsPQ.pdf"), width = 7, height = 7)
...
#output txt file with results
write.table(misClustData, file = paste(outDir, "simCompMethodsSparse.txt"))

#create figure
pdf(paste(outDir, "simCompMethodsSparse.pdf"), width = 7, height = 7)
    xyplot(misClustRate ~ deltaPQ, group = group, type = "b", pch = 1:4,
           data = misClustData, ylab = "Average Mis-clustering Rate",
           xlab = "p - q", lwd = 2, key = list(
                               text = list(c("CASC","CCA","SC-L","SC-X")),
...
# ---------------------------------------------------------------------
# comparison of misclustering using CASC, CCA, L, X versus n for sparse case
# ---------------------------------------------------------------------
#model parameters
nBlocks = 2
nCovs = 2
...
for(iter in 1:nIter) {
    ...
    for(nNodes in nNodesSeq) {
        ...
        nMembers = c(nNodes/2, nNodes/2)
        p = .03 * (1000/log(1000)) * (log(nNodes)/nNodes)
        q = .5 * p
